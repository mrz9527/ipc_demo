...
int main(int argc, char **argv)
{
    ...
    // 创建共享内存1、2。对于同一块共享内存，服务端的读，就是客户端的写。
    int wshm_id = create_shm_by_ftok("./1.txt", 'r',  SHM_SIZE, IPC_CREAT | 0777);
    int rshm_id = create_shm_by_ftok("./1.txt", 'w',  SHM_SIZE, IPC_CREAT | 0777);
    ...
    // 交互进程号，用的是同一个共享内存，所以这里临时用了读共享内存来发送当前进程号
    // 获取服务端进程号
    server_pid = rshmbuf->pid;
    // 向共享内存写入自己的进程号
    rshmbuf->pid = getpid();
    // 通知服务端
    kill(server_pid, SIGUSR2);
    ...
    // 设置信号处理函数
    signal(SIGUSR1, sigReadBufHandler);
